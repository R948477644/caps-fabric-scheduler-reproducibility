...
const config = {
    port: Number(process.env.ED_MVCC_DISPATCHER_PORT || 8087),
    maxQueue: Number(process.env.ED_MVCC_MAX_QUEUE || 500),
    maxActive: Number(process.env.ED_MVCC_MAX_ACTIVE || 32),
    ...
    endorsingPeers: (process.env.FABRIC_ENDORSING_PEERS || 'peer0.org1.example.com:7051,peer0.org2.example.com:9051')
        ...
        .map(peer => peer.trim())
        .filter(Boolean),
    settleMs: Number(process.env.ED_MVCC_SETTLE_MS || 0)
};
...
class EarlyMvccDispatcher {
    ...
    enqueue(task) {
        ...
        const conflict = this.findPendingConflict(task);
        if (conflict) {
            this.metrics.rejected += 1;
            this.metrics.earlyConflictRejected += 1;
            this.metrics[conflict.metric] += 1;
            sendJson(task.res, 429, {
                ok: false,
                error: 'ed_mvcc_pending_conflict',
    ...
    findPendingConflict(task) {
        for (const key of task.readSet || []) {
            if ((this.pendingWriters.get(key) || 0) > 0) {
                return { type: 'read_after_pending_write', metric: 'readWriteConflicts', key };
            }
        }
        for (const key of task.writeSet || []) {
            if ((this.pendingReaders.get(key) || 0) > 0) {
                return { type: 'write_after_pending_read', metric: 'writeReadConflicts', key };
            }
            if ((this.pendingWriters.get(key) || 0) > 0) {
                return { type: 'write_after_pending_write', metric: 'writeWriteConflicts', key };
            }
        }
        return null;
    }
    ...
    reserve(task) {
        for (const key of task.readSet || []) {
            this.increment(this.pendingReaders, key);
        }
        for (const key of task.writeSet || []) {
            this.increment(this.pendingWriters, key);
        }
        task.reserved = true;
    }
    ...
    release(task) {
        ...
        for (const key of task.readSet || []) {
            this.decrement(this.pendingReaders, key);
        }
        for (const key of task.writeSet || []) {
            this.decrement(this.pendingWriters, key);
        }
        task.reserved = false;
    }
    ...
    drain() {
        ...
        while (this.active < config.maxActive && this.queue.length > 0) {
    ...
    async runTask(task) {
        ...
        try {
            ...
            console.error('ED-MVCC task failed:', err && (err.stack || err.message) || err);
    ...
    snapshot() {
        ...
        return {
            ...m,
            active: this.active,
            pendingReadKeys: this.pendingReaders.size,
            pendingWriteKeys: this.pendingWriters.size,
...
async function main() {
    ...
    const dispatcher = new EarlyMvccDispatcher(contract);

    const server = http.createServer(async (req, res) => {
        if (req.method === 'GET' && req.url === '/health') {
            sendJson(res, 200, {
                ok: true,
                mode: 'ed-mvcc-reimpl-dispatcher',
        ...
        if (req.method === 'POST' && (req.url === '/transfer' || req.url === '/submit')) {
            ...
            if (!task) {
                console.error('Invalid ED-MVCC payload:', JSON.stringify(payload));
    ...
    server.listen(config.port, () => {
        console.log(`ED-MVCC reimplementation dispatcher listening on ${config.port}`);
